...
import (
	...
	"gobox/internal/gitutil" // Import our internal gitutil package
	"gobox/internal/parser"  // Import our internal parser package
	"gobox/pkg/task"         // Import our pkg/task package
)
...
func main() {
	if len(os.Args) != 2 {
		fmt.Println("Usage: gobox <markdown_file_path>")
		os.Exit(1)
	}
	filename := os.Args[1]

	tasks, err := parser.ParseMarkdownFile(filename) // Use parser package
	if err != nil {
	...
	err = parser.UpdateMarkdown(filename, *nextTask, commitsDuringTask) // Use parser package
	if err != nil {
